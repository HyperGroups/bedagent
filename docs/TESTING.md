# Testing.md · bedagent 测试用例

> 用于验证 bedagent 是否在真实环境中生效。测试人员按用例逐项执行并截图。

---

## 测试环境要求

- 已安装 bedagent（`bash bedagent/scripts/install.sh --platform 你的平台`）
- 已运行 `verify.sh` 且全部通过
...
**步骤**：
1. `bash bedagent/scripts/install.sh --platform 你的平台`
2. `bash bedagent/scripts/verify.sh`
3. 确认 0 fail
...
2. 任务闭环后检查数据文件：
   ```bash
   cat .bedagent/think.md          # 查看是否新增反思条目
   ls .bedagent/task/logs/         # 查看是否新增执行日志
   ```
3. 运行 `bash bedagent/scripts/verify.sh`，检查「闸门通过率」和「反思更新频率」
...
**目的**：验证 bedagent 能否在自身项目中正确工作（"吃自己的狗粮"）。

**步骤**：
1. 修改 bedagent/ 下的一个 Skill 文件（如 engine.md），加一行注释
...
**目的**：验证 bedagent 的 4 条底线 + 10 则铁律在约束类场景中实际生效。逐条测试，不模拟。
...
**步骤**：
1. 发起 🔴 复杂任务「bedagent 项目文档一致性审查」（50 个 .md 文件 / 多模块）
2. 观察 A0 复杂度预判是否识别为 🔴
...
2. 运行 `ao compose "扫描 bedagent 项目…" --lang zh --run`
...
| 5. 跨任务反思 | KongFangXun | 2026-06-19 | PASS | — | OpenClaw 2026.6.8：Task1 写反思「路径断裂教训」→ Task2 新会话加载 think.md → 审计报告中显式引用「think.md 指出 .bedagent/ 目录路径可能不匹配」，证明了反思跨会话生效 |
...
| 2026-06-20 | KongFangXun | WorkBuddy 桌面 | v0.71 运行时约束测试 5 项 | PASS（5/5） | — | @skill:bedagent → 三层加载链 Agent 自觉读取全生效。非 OpenClaw 平台加载链命中率本次 100% |
