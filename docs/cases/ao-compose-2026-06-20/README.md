...
```bash
# 1. OpenClaw 部署 bedagent v0.71（完整能力）
bash bedagent/scripts/install.sh --platform openclaw
# → bedagent/ 同步到 ~/.openclaw/skills/bedagent/
# → scripts/ 同步到 ~/.openclaw/scripts/
# → Hook handler.ts 部署到 ~/.openclaw/hooks/bedagent-load-chain/
...
# 4. 验证部署
bash bedagent/scripts/verify.sh --quiet   # 33 项全部通过
...
| 组件 | 版本/配置 |
|------|------|
| bedagent | v0.71（install.sh --platform openclaw 完整部署） |
| ao compose | npm 全局 `agency-orchestrator` |
| LLM | DeepSeek Chat（deepseek-chat） |
| 并发度 | 4 |
| 角色库 | 216 个角色可选 |
| Hook | bedagent-load-chain（handler.ts 双路径 fallback，优先 constitution 路径） |
...
openclaw agent --session-key agent:default:bedagent-v071-test \
  --message "请读取 bedagent/SKILL.md，然后告诉我 bedagent 的 4 条底线分别是什么。"
# → ✅ 正确列出 4 条底线
