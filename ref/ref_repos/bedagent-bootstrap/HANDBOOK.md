...
>
> v0.91 · 2026-06-25 · bedagent contributors
...
bedagent 是一个床上指挥版 Agent 纪律层：参考 sofagent 的主体设计，把“先读后写、验证再干、谨慎修改、任务复盘”这些习惯包装成可安装、可审计、可反思的规则链。

它不追求让模型变聪明，而是让 Agent 更守规矩：你负责定义目标和边界，Agent 负责推进任务、记录过程、在关键节点回头检查。

为什么叫 bedagent？bed + agent，合起来就是「床上特工」。想表达的意思很直接：希望有一天能躺在床上指挥智能体，Agent 就把活干完了。

这不是模型框架，也不是 Skills 商店。它更像一套 Agent 行为纪律和任务闭环模板：核心靠 Markdown 约束，增强靠脚本、Hook、编排和提交时审计。

当前版本继承了 sofagent 的历史文档、脚本结构和实验记录，并完成 bedagent 命名与语义迁移。后续还需要补充 bedagent 自己的使用数据。

如果你也在折腾 OpenClaw / WorkBuddy / Claude Code / Codex / Hermes Agent，希望它能让你少盯屏幕，多把活交给 Agent。
...
| 怎么装 | `bash bedagent/scripts/install.sh --platform 你的平台` | §五 |
...
> 💡 Context → Harness → Loop 的演进逻辑，以及 bedagent 为什么在这三个阶段中选了 Harness + Loop 双引擎，见 [ARCHITECTURE.md](./ARCHITECTURE.md#一为什么会有-bedagent)。
...
bedagent 概念不少——宪法、铁律、加载链、编排引擎、断路器……你不需要全部理解才能用起来。先记住两件事：

1. **核心**：4 底线 + 10 铁律 + 三层加载链（SKILL → think → rules）。这些**所有平台都生效**，是你装了 bedagent 就能拿到的纪律约束
...
bedagent 每次对话启动时，先加载 3 个文件作为常驻地基——然后再判断任务复杂度：
...
> 📎 orchestrator/ 不参与初始加载——由 SKILL.md 判断任务需要编排时按需触发。三层约束注入：第 1 层由 skill 系统自动注入；第 2、3 层在 OpenClaw 上由内部 hook `bedagent-load-chain`（agent:bootstrap 事件）注入，其他平台由 Agent 主动 Read。
...
在开始目标确认之前，先过一道关卡——**让用户理解 bedagent 能做什么。**

bedagent 就是一个在电脑前干活的人——能通过文字输入输出来完成的都能做（查资料、写文档、分析数据、写代码、做报表），走不出屏幕（插 U 盘、打印、面对人沟通）。
...
| `workbuddy` | 部署 SKILL.md → `~/.workbuddy/skills/bedagent/`（也可通过技能市场安装） |
...
未指定 `--platform` 时自动探测。OpenClaw 用户可直接 `bash bedagent/scripts/install.sh`。

> 💡 WorkBuddy 用户：技能市场安装，或 `git clone` + `bash bedagent/scripts/install.sh --platform workbuddy`。装好后 SKILL.md 首次加载自动完成初始化（B1 步创建 `.bedagent/` 数据目录）。

**怎么验证装好了**：跑 `bash bedagent/scripts/verify.sh`，自动检查 9 类 24+ 项。加 `--json` 可集成到 CI/CD。如果没通过，去 §六 FAQ 看「Agent 不遵守铁律」那条排查。
...
**复杂任务时建议**：任务前加 `@skill:bedagent` 作为显式锚点，提高 Agent 走完加载链的概率。
...
Agent 改完代码 commit 了——它遵守了铁律吗？`bedagent-audit` 帮你回答这个问题。

它是一个独立 CLI 工具（TypeScript），扫描 git diff 和 `.bedagent/task/logs/` 操作日志，对照四条铁律逐条判定：
...
> 💡 详细设计见 [DEVELOPMENT §八](./DEVELOPMENT.md#八确定性纪律检查与提交时审计)，源码在 `bedagent-audit/src/`。
...
> 📎 能力边界（bedagent 能做什么、不能做什么）详见 §四「能力边界」。
...
| 问题 | 怎么办 |
|------|------|
| Agent 不遵守铁律 | 检查文件位置；把最关键规则写到 rules.md。非 OpenClaw 平台（WorkBuddy / Codex / Hermes Agent / Claude Code）若 Skill 未自动加载，在对话中手动 `@skill:bedagent` 触发入口流程 |
...
Addy Osmani 在 Loop Engineering 里泼了三盆冷水——不是 bedagent 的设计缺陷，是任何自动化系统的宿命。bedagent 能做的是让你知道它们存在，不能替你解决。
...
两个人搭一模一样的 bedagent，可能得到完全相反的结果——一个用它加速自己深刻理解的工作，另一个用它逃避理解。bedagent 分不出区别，你分得出。
...
感谢这些项目、产品和作者，没有他们就没有 bedagent。

### 最早要谢的

- **[OpenClaw](https://github.com/openclaw/openclaw)** by Peter Steinberger — 整个 bedagent 的基石。从上下文加载到 Hook 触发、从 Skill 注入到 Session 管理，整套体系都建立在 OpenClaw 的能力之上。没有 OpenClaw，这本 Handbook 一页都写不出来
- **[DeepSeek V4 Pro](https://api-docs.deepseek.com/zh-cn/)** — 这本 Handbook 和所有 bedagent 文件都是它与 GLM-5.2 配合生成的。一个产品经理能做完这件事，全靠它们
