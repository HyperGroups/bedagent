...
<!-- TODO: demo.gif — 15s side-by-side: bare Agent going off-rails vs bedagent-constrained Agent -->

> bed + agent — because one day, we want to stay in bed and direct agents to finish the work.
> v0.91 · 2026-06-25

> 📄 **License**: MIT. Code, docs, templates — use freely, just keep the copyright notice.

bedagent is a bed-themed adaptation inspired by [sofagent](https://github.com/HyperGroups/sofagent). It keeps the core idea — a cross-platform discipline layer for agents — and remaps the product metaphor from "sit back on the sofa" to "direct agents from bed."

The imported historical docs and experiments remain useful design context; bedagent-specific evidence still needs to be collected.
...
> ⚠️ bedagent is a **soft constraint layer** — it works when the agent reads and chooses to comply. It is not hard-coded enforcement. Effectiveness depends on context window capacity and model capability. See [Design §三](./ARCHITECTURE.md).
...
> ⚠️ **Known limitations**: No third-party usage data yet; reflection is LLM self-assessment with no objective baseline; the Loop Agent is not a separate process; file-only constraints depend on agent cooperation; data stored in plaintext (task/logs + think.md contain task records, unencrypted); not a multi-user system (sharing .bedagent/ cross-contaminates experience). See full list in [LIMITATIONS.md](./LIMITATIONS.md#known-limits).
...
| WorkBuddy | Layer 1 via @skill injection, Layers 2/3 agent-cooperative | `ao compose` available (requires npm install) | Medium — needs manual @skill:bedagent |
...
> ⚠️ **Non-OpenClaw reality check**: Orchestration engine / Hook / Circuit breaker — all three core capabilities are OpenClaw-only. If you're not on OpenClaw, bedagent delivers about 30% of its full value (constitution layer only). This is not a bug — it's an architectural reality. v0.8 daemon will improve the loading chain, but orchestration and hooks remain OpenClaw-exclusive. See [Design §三 Platform Dependencies](./LIMITATIONS.md#平台依赖).

> 📎 "Seed instruction" — a single line pasted into the agent's memory file (e.g. CLAUDE.md / AGENTS.md / SOUL.md) telling the agent to read bedagent constraint files at startup. **This is not automation — it's a manual sticky note.** OpenClaw and WorkBuddy auto-load via their skill mechanisms; no seed instruction needed.
...
```bash
bash bedagent/scripts/install.sh --platform your-platform
```

| Platform | Command | Notes |
|------|------|------|
| **OpenClaw** | `bash bedagent/scripts/install.sh` | Auto-detected, full deployment (constitution + hook + circuit breaker) |
| **WorkBuddy** | `bash bedagent/scripts/install.sh --platform workbuddy` or via skill marketplace | Deploys SKILL.md to `~/.workbuddy/skills/bedagent/` |
| **Claude Code** | `bash bedagent/scripts/install.sh --platform claude` | Deploys constitution + outputs seed instruction (paste into CLAUDE.md) |
| **Codex** | `bash bedagent/scripts/install.sh --platform codex` | Deploys constitution + outputs seed instruction (paste into AGENTS.md) |
| **Hermes Agent** | `bash bedagent/scripts/install.sh --platform hermes` | Deploys constitution + outputs seed instruction (paste into SOUL.md) |

> Auto-detects platform if `--platform` is omitted.

**What install.sh modifies**:
- OpenClaw: writes to `~/.openclaw/rules.md`, `~/.openclaw/skills/bedagent/` (6 skill .md + scripts/ + data/ + rules.md)
- WorkBuddy: writes to `~/.workbuddy/skills/bedagent/` (SKILL.md + subdirectories)
...
Open your agent client. Try a task that needs multi-step decomposition (this is where bedagent's orchestration shines):
...
```bash
ls .bedagent/task/logs/        # execution logs organized by year-month
cat .bedagent/think.md         # auto-generated reflection summary
```
...
└── bedagent/           ← core deployment files
    ...
    ├── hooks/bedagent-load-chain/  ← OpenClaw hook (injects layers 2 and 3)
    ├── scripts/        ←   install / verify / uninstall / task-record / task-orchestrate / cleanup / audit
    └── images/
```

After installation, `.bedagent/` is auto-generated (agent work notes: think.md reflections + orchestrator/ config + scoring.md ratings + task/ audit logs). Every task is auto-logged; cross-task reflection accumulates automatically.

> 💡 6 skills (1 main + 5 sub) + scripts each do one thing. Reuses 4 open-source projects for orchestration/roles/skill discovery. bedagent itself only writes the governance layer.
...
- [OpenClaw](https://github.com/openclaw/openclaw) by Peter Steinberger — the foundation bedagent runs on
