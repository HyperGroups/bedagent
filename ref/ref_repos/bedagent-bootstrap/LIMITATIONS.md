# bedagent Limitations

> 诚实坦白：已知局限。这里不卖「企业级安全」或「军事级加密」——只写 bedagent 做不到什么、为什么做不到、等什么才能做到。
>
...
bedagent 的核心机制是 MD 文件注入 Agent 上下文。这意味着：约束力 = Agent 的注意力 × 平台的加载可靠性。上下文窗口太小约束可能被截断；选择性忽略长文本（Lost in the Middle），中间铁律可能漏掉；约束机制依赖 Agent 配合——它必须"愿意读"。
...
**三层加载链在非 OpenClaw 平台上不可靠。** Agent 声称"跑了 bedagent"，实际可能只读了 1/3（宪法层被跳过）。
...
实测数据：两轮 WorkBuddy 新会话测试，加载链命中率分别为 1/3 和 0/3。OpenClaw 侧通过内部 hook `bedagent-load-chain`（agent:bootstrap 事件）注入第 2、3 层，无此问题。

**用户侧缓解**：在复杂任务前加 `@skill:bedagent` 作为显式锚点，可提高 Agent 注意到约束的概率——但非强制保证。详见 HANDBOOK。根治需等各平台支持类似 Hook 机制。
...
bedagent 的 `scoring/` + `think.md` 自进化机制当前处于**第一阶段：经验记录**。评分基于使用频率和单次任务表现——这正是学术界警告的「单次轨迹不可靠」问题：一次任务失败可能是环境异常或模型波动，不意 Skill 本身有问题。
...
Loop Engineering 的标志性场景是「每天早上 8 点自动扫 CI」。bedagent 目前只有「每次对话启动」这一种触发方式。OpenClaw 不支持 cron 级定时任务。
...
SKILL.md B1 步用 bash heredoc 创建 `.bedagent/` 数据目录和核心文件。OpenClaw / WorkBuddy / macOS/Linux 默认有 bash，但 Windows 或受限沙盒环境可能没有。
...
> 📊 这个防护体系不是过度设计。Writer 2026 年初调查显示：35% 的公司无法有效关停失控的 Agent，97% 高管称已部署 AI Agent 但实际回报率仅 29%。bedagent 的三层过滤网每层都对应一种实际失控场景——要解决的是「用了 AI 之后谁来管、怎么管」。
...
bedagent 目前没有这个能力：Hook 是 OpenClaw 配置层的静态设置，Skill 运行时无法动态注册。现阶段 Skill 的安全约束只能靠静态 rules.md + OpenClaw `tools.loopDetection` 兜底。

**等什么**：OpenClaw 支持 Skill 级动态 Hook，或者 bedagent 在 Skill 入口分发器层面实现自己的沙盒层。

---

### 🧩 不是分布式系统

bedagent 跑在单个 Agent 里——没有 agent-to-agent 通信，没有多实例协调，没有分布式状态管理。子 Agent 是 OpenClaw 的 session 隔离，不是真正的独立 Agent 进程。
...
bedagent 跑在单个 Agent 里，.bedagent/ 是单用户工作目录。如果多用户共享同一 .bedagent/（比如团队共享仓库），一个人的错误反思会通过 think.md 污染所有人的判断。这不是 bug——是单 Agent 设计和多用户使用之间的天然冲突。

多用户场景建议：每人独立 .bedagent/（每人独立工作目录），或等 v0.7x 的多用户方案。
...
bedagent 的应对：think.md 的置信度渐进（0.3→0.5→0.7）和 30 天无触发衰减，本身就是对迷信仪式的过滤器——靠单次巧合涨不到高置信度。但更根本的解法是定期审计——翻 task/logs 对照 think.md 的反思来源，把偶然成功标记剔除。这是人要做的事，自动化做不到。
...
| Skill 自启 | ✅ | ✅ | ❌ | ❌ | ❌ |
| 加载链脚本 | ✅ 内部 hook（bedagent-load-chain） | ❌ 无Hook，C步Read替代 | ❌ | ❌ | ❌ |
...
> - **第 2、3 层（think.md + rules.md）**：OpenClaw 由内部 hook `bedagent-load-chain`（agent:bootstrap 事件触发）注入；其他平台由 Agent 主动 Read
>
...
> 💡 **设计妥协：MD 强约束对标 Hook 机制**。bedagent 的三层闸门在概念上对标 Cloud/Agent 的 Hook 机制——回复前闸门 ⓪ = pre-tool-use（工具调用前检查），task-closure ②→⑤ = post-tool-use（任务结束后自动沉淀），闭环信号 = stop event（任务完成触发）。但受限于跨五平台兼容性（WorkBuddy/Codex/Hermes/Claude Code 不支持 Shell 级 Hook 拦截），这些只能通过 MD 强约束 + ⛔ 硬出口 + 兜底检查来模拟 Hook 行为。只有 OpenClaw 平台通过内部 hook `bedagent-load-chain`（2026.6.x）实现了真正的 Hook 级注入。
...
v0.85 确立新方向：**提交时审计**（bedagent-audit）。不依赖 Agent 配合——看的是已经发生的 git diff。但这不意味着放弃运行时治理——两者互补。当前阶段运行时治理仍是主体，提交时审计是 v0.91 的 MVP 方向。
...
这是真实问题。bedagent 的概念密度对新手不友好——一个开发者想贡献代码，先要搞懂十几个概念之间的关系。v0.85 的缓解措施是在 CONTRIBUTING 里加「贡献者 10 分钟速览」（只看这 3 个文件，改这 2 个脚本，跑这 1 条命令验证）。根治需要概念精简——但当前阶段验证纪律层增量优先于概念重构。
