...
评审落地 + bedagent-audit MVP + 文档瘦身——两份独立评审（GLM-5.2 72 分 + DeepSeek V4 Pro 62 分）的共识项统一落地，同时启动核心战略转向：提交时审计。

### 🔴 bedagent-audit MVP（提交时审计）
...
| 2 | `--lite` 模式跳过 Step 5b 导致 `$BEDAGENT_DATA` 未创建，think.md 写入失败 | think.md 写入前 `mkdir -p` |
| 3 | 8 个脚本各自硬编码 `BEDAGENT_DATA="${PWD}/.bedagent"`，`--project-dir` 装在 A 目录从 B 目录跑找不到数据 | `config.sh` 新增 `_bed_find_data_dir()` 统一解析，4 级优先链 |

**连带修复**：`config.sh` 布尔型配置不再被空值覆盖——rules.md 无匹配时保留已有环境变量。

### 证据体系

Case 012（社区 A/B 测试）：社区成员小嘉用 DeepSeek Reasoner 跑 5 个代码重构任务真 A/B 对照——bedagent 组首次无 bug 率 5/5，裸 Agent 组 4/5；陷阱注释全保留 vs 部分移除；类型完整 interface vs `any` 绕过。5 维度全正向。
...
2. bedagent Lite（30 秒只装宪法层，`--lite` 参数已实现）
3. bedagent-audit 方向确立（提交时审计）
...
| 平台 | 模式 | bedagent 生效？ | 明确增量 | 核心限制 |
|------|------|:---:|:---:|------|
| WorkBuddy | 对话（skill 自觉加载） | ⚠️ 部分 | 1/10 | B 组加载链已含 bedagent 思想，非裸模型 |
...
> bedagent 的有效边界 = Agent 会话是否经过 bootstrap 生命周期。代码重构场景的增量在纪律层（减少误改 + 验证习惯），不在能力层。
...
v0.84 期间用 v0.81-v0.83 代码跑了 5 组 A/B，最重要的发现不是"CLI无效"或"WorkBuddy只有1/10"，而是**我们一直在用错误的尺子量 bedagent**。benchmark 10 任务中 7 个测约束层（拒绝/追问），但约束层被三层压缩（模型安全训练 + 用户加载链 + CLI绕过）——增量天花板很低。独立测试者用纪律层尺子（先读再用/验证再干/谨慎修改）量出了真实增量：纪律性 +2，首次通过率 +40%。

**bedagent 真正的差异化在纪律层，不在约束层。** 约束层是 base layer（★★☆），纪律层是 value layer（★★★★★）。
...
企业合规三件套：日志脱敏（task-record.sh sanitize()）+ 数据保留策略（cleanup.sh）+ 审计日志（audit.sh）+ 共享配置层（lib/config.sh）。v0.70.1 修 Codex 平台兼容性（BEDAGENT_DATA 未初始化 + verify.sh 误查 OpenClaw Hook）。
