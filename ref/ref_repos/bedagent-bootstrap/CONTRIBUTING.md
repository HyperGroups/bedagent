...
| 你想... | 怎么做 |
|------|------|
| 报 Bug / 提想法 | → [开 Issue](https://github.com/HyperGroups/bedagent/issues/new/choose) |
| 不知道怎么用 | → [Discussions 去问](https://github.com/HyperGroups/bedagent/discussions) |
...
| 顺序 | 文件 | 看什么 | 约几分钟 |
|:--:|------|------|:--:|
| 1 | [SKILL.md](./bedagent/SKILL.md)（宪法内联） | 4 底线 + 10 铁律——项目的灵魂，所有概念从这里展开 | 3 min |
...
| 脚本 | 改什么 | 难度 |
|------|------|:--:|
| `bedagent/scripts/install.sh` | BSD/macOS 兼容性修复（见 [MEMORY.md 脚本工程教训](./.workbuddy/memory/MEMORY.md)） | ⭐⭐ |
| `bedagent/scripts/verify.sh` | 新增检查项（加 `--quick` 模式的检查项） | ⭐ |

**跑 1 条命令验证**：

```bash
bash bedagent/scripts/install.sh && bash bedagent/scripts/verify.sh
```

通过就可以提 PR。

> 💡 你不需要会写代码。跑一周 bedagent，回来告诉我们发生了什么——不管好坏。
...
→ 直接开 [Issue](https://github.com/HyperGroups/bedagent/issues)。
...
Handbook（`HANDBOOK.md`）有一项硬性约束：**改手册必须同步改 `bedagent/` 模板。反过来也一样。**
...
1. **先改 `bedagent/`**（工作区源文件，唯一权威）
2. **重新安装同步**：`bash bedagent/scripts/install.sh --platform openclaw`（覆盖全部 Skill 文件到安装位置）
...
# OpenClaw：一键安装
bash bedagent/scripts/install.sh

# WorkBuddy：复制到 skills 目录
cp -r bedagent/ ~/.workbuddy/skills/bedagent/

# 验证安装
bash bedagent/scripts/verify.sh
```

> 改完 Skill 文件后，WorkBuddy 用户需手动同步到 `~/.workbuddy/skills/bedagent/`。OpenClaw 用户重新运行 install.sh 即可。
...
| ④ 安装副本同步 | `cp -r bedagent/ ~/.workbuddy/skills/bedagent/` | 工作区 → 安装副本 |
...
> 你不需要会写代码。跑一周 bedagent，回来告诉我发生了什么——不管好坏。
...
bedagent 当前维护者为孔放勋一人。我们正在寻找愿意深度参与的 Co-maintainer。
