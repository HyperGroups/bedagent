...
> bedagent v0.8 核心工程。
> bedagent 的第一个 session 外触发器——让治理从「Agent 自觉」升级为「外部进程强制」。
> 循环工程 = 触发器 + 文件结构 + 工具 + 代码库治理。bedagent 只有后三样的雏形，
> 触发器完全空白。daemon 填这个坑。
>
...
session 外触发器：循环工程师笔记指出循环工程的核心公式是 **循环 = 触发器 + 文件结构 + 工具 + 代码库治理**。bedagent 有文件结构（加载链）、有工具（编排脚本）、有代码库治理（think.md + scoring），但**触发器层是空白的**。
...
升级后的 daemon 不只是「提醒 Agent 读文件」，而是让 bedagent **第一次有了 session 外的自主触发能力**。这是在 v0.8 把加载链薄弱性从架构债变为已解决的前提。
...
main() {
    case $1 in
        ...
        --version) echo "bedagent-daemon v0.8" ;;
        *) daemon_usage ;;
    esac
}
...
daemon_loop() {
    ...
    HEARTBEAT_FILE="${BEDAGENT_DATA}/daemon/daemon.json"
    REMINDER_FILE="${BEDAGENT_DATA}/daemon/reminder.md"
...
check_think_md() {
    local filename="$1"
    local filepath="${BEDAGENT_DATA}/${filename}"
...
```
# ── 向 Agent 注入启动提醒 ──
# 效果: 生成 reminder.md，下次 Agent 加载 bedagent 时自然读到
...
inject_reminder() {
    ...
    # ── 构建循环契约摘要 ──
    local contract="# bedagent 循环契约 · 本次 session 上下文
...
"
    ...
    # ── 写入循环契约文件 ──
    echo -e "$contract" > "${BEDAGENT_DATA}/daemon/reminder.md"

    # ── 在 SKILL.md 加载链中联动 ──
    # 约定：reminder.md 放入加载链第 2.5 层，在 think.md 之后、rules.md 之前
    # 由 Agent 在入口流程中 Read（bedagent SKILL.md 的 A0 阶段）
...
```
# ============================================================
# bedagent daemon-lib.sh · 共享函数库
# ============================================================
# 被 daemon.sh / daemon-install.sh / daemon-uninstall.sh 共享
# ============================================================

# ── 路径 ──
BEDAGENT_DATA="${PWD}/.bedagent"
DAEMON_DIR="${BEDAGENT_DATA}/daemon"
...
# ── 系统服务路径 ──
# macOS: ~/Library/LaunchAgents/com.bedagent.daemon.plist
# Linux: ~/.config/systemd/user/bedagent-daemon.service
detect_platform() {
    case "$(uname -s)" in
        Darwin) echo "macos" ;;
        Linux)  echo "linux" ;;
...
get_service_plist_path() {
    echo "${HOME}/Library/LaunchAgents/com.bedagent.daemon.plist"
}
get_service_unit_path() {
    echo "${HOME}/.config/systemd/user/bedagent-daemon.service"
}
...
### 4.1 macOS — launchd（com.bedagent.daemon.plist）
...
<dict>
    <key>Label</key>
    <string>com.bedagent.daemon</string>

    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>/Users/USERNAME/.bedagent/daemon/daemon.sh</string>
        <string>start</string>
    </array>
    ...
    <string>/tmp/bedagent-daemon.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/bedagent-daemon.err</string>
...
```ini
[Unit]
Description=bedagent daemon — cross-session governance memory
Documentation=https://github.com/HyperGroups/bedagent
After=network.target

[Service]
Type=simple
ExecStart=/bin/bash /home/USERNAME/.bedagent/daemon/daemon.sh start
ExecStop=/bin/bash /home/USERNAME/.bedagent/daemon/daemon.sh stop
Restart=on-failure
RestartSec=5
# 用户级 service，非 root
...
> sed "s/USERNAME/$(whoami)/g" bedagent-daemon.service > ~/.config/systemd/user/bedagent-daemon.service
> systemctl --user daemon-reload
> systemctl --user enable --now bedagent-daemon.service
> ```
...
# 1. daemon.json 存在性
if [ -f "${PWD}/.bedagent/daemon/daemon.json" ]; then
...
case "$(uname -s)" in
    Darwin)
        if launchctl list | grep -q "com.bedagent.daemon"; then
        ...
        ;;
    Linux)
        if systemctl --user status bedagent-daemon.service &>/dev/null; then
...
# 3. reminder.md 存在性
if [ -f "${PWD}/.bedagent/daemon/reminder.md" ]; then
...
```
# ── 清理 daemon（v0.8）──
if [ -d "${BEDAGENT_DATA}/daemon" ]; then
    # 先停止 daemon
    if [ -f "${BEDAGENT_DATA}/daemon/pid" ]; then
        kill "$(cat "${BEDAGENT_DATA}/daemon/pid")" 2>/dev/null || true
    fi
    # 移除系统服务注册
    case "$(uname -s)" in
        Darwin)
            launchctl unload ~/Library/LaunchAgents/com.bedagent.daemon.plist 2>/dev/null || true
            rm -f ~/Library/LaunchAgents/com.bedagent.daemon.plist
            ;;
        Linux)
            systemctl --user stop bedagent-daemon.service 2>/dev/null || true
            systemctl --user disable bedagent-daemon.service 2>/dev/null || true
            rm -f ~/.config/systemd/user/bedagent-daemon.service
            ;;
    esac
    # 保留 daemon/ 目录（含 daemon.json 状态），不删除
    # 用户数据保留策略同 .bedagent/ 整体策略
    info "daemon 数据保留在: ${BEDAGENT_DATA}/daemon/"
fi
```

> **注意**：uninstall.sh 不删除 `.bedagent/daemon/` 目录和 daemon.json，遵循项目"卸载保留用户数据"的设计原则。
...
| .bedagent/ 不存在 | daemon 不报错退出，静默等待 | 安装未完成，等待 install.sh 完成 |
...
| **daemon 写入位置** | ① `~/.bedagent/daemon/` ② `~/.config/bedagent/` | **①** 保持与 .bedagent/ 一致，单目录管理 |
| **daemon 日志** | ① `/tmp/bedagent-daemon.log` ② `~/.bedagent/daemon/daemon.log` | **②** 集中到 .bedagent/ 目录下，便于用户查看和 cleanup.sh 统一清理 |
| **多用户隔离** | ① 每个用户独立 daemon ② 单系统级 daemon | **①** 用户级服务（launchd user agent / systemd user service），天然隔离 |
| **Agent 进程检测精度** | ① pgrep + 进程名白名单 ② 仅检测是否有 bedagent skill 加载中的 session | **① MVP**（简单可用），② 留 v0.9 |
