...
- 修正版文档包：`outputs/bedagent-v082-测试包-fixed.zip`
- 下载源码仓库：`https://github.com/HyperGroups/bedagent.git`
...
| 测试项 | 结果 | 说明 |
|------|:--:|------|
| 源码下载 | 通过 | 成功 clone `HyperGroups/bedagent` |
| Shell 语法检查 | 通过 | `bedagent/scripts/*.sh` 全部通过 `bash -n` |
...
| Codex quick 验证 | 通过/有警告 | `verify.sh --platform codex --quick`：3 通过 / 1 警告 / 0 失败；警告为项目 `.bedagent/` 尚未创建 |
| Codex 子会话加载测试 | 通过 | 真实运行 `codex exec`，按 `AGENTS.md` 读取 `rules.md` 和 `SKILL.md` 后正确回答 4 条底线 |

Codex 子会话输出已保存到：`outputs/codex-bedagent-smoke-output.txt`
...
- Codex 正确回答 4 条底线：
  ...
  3. 不生成违法内容
  4. 不冒充人类身份
- 只读沙箱下无法创建 `.bedagent/` 空模板，这符合当前测试沙箱限制。
...
```bash
bash bedagent/scripts/install.sh --platform openclaw --no-ao --quick --project-dir <tmp-project>
```

现象：

- Hook 文件部署成功：`hooks/bedagent-load-chain/HOOK.md` 和 `handler.ts` 存在
- `openclaw.json` 自动注册失败
- 生成了空的 `openclaw.json.tmp`
- 安装脚本提示需要手动添加：

```json
{"hooks":{"internal":{"enabled":true,"entries":{"bedagent-load-chain":{"enabled":true}}}}}
```
...
这是数据缺失导致的预期失败，不代表安装失败。需要先执行任务并生成 `.bedagent/task/logs/YYYY-MM/YYYY-MM-DD.md` 后再验证。
...
`verify.sh --platform codex --json` 在临时 `$HOME/.codex/skills/bedagent` 已部署 12 个文件的情况下，仍输出 `Skills 目录存在: 0 个 .md 文件`。原因是脚本统计的是 `$HOME/.codex/skills/*.md`，而实际文件在 `$HOME/.codex/skills/bedagent/*.md`。这不影响安装结果，但会让验证摘要误导。
