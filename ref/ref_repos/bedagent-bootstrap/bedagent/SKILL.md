...
slug: bedagent
version: 0.91
displayName: bedagent
description: >
  当你的 Agent 反复偏离目标、任务越做越复杂、刚踩过的坑下次还踩 —— bedagent 能约束其行为、拆解复杂任务、从错误中沉淀教训。
image: images/bedagent.svg
...
> **平台定位**：第 1 层所有平台强制生效（skill 机制保证）；第 2、3 层依赖 Agent 自觉 Read。OpenClaw 通过内部 hook（`bedagent-load-chain`，agent:bootstrap 事件触发）进一步强化后两层。
...
| 2 | `{BEDAGENT_DATA}/think.md` | Agent 主动 Read | 反思区（上次踩了什么坑）| 任务完成后创建 |
| 3 | `~/.openclaw/skills/bedagent/rules.md` | Agent 主动 Read | 你的运行规范（最高优先级，可覆盖第 1 层）| 跳过（未配置）|
> 💡 `~/.openclaw/rules.md` 留给用户自定义，bedagent 不再部署到此路径。|

> 💡 `{BEDAGENT_DATA}` = `${PWD}/.bedagent`（当前工作目录下的 .bedagent/ 数据目录）。
> 💡 `{OPENCLAW_SCRIPTS}` = 优先 `${HOME}/.openclaw/scripts/`；若不存在则 Agent 自行搜索 `bedagent/scripts/`（项目目录下的脚本）。
...
> 你的平台可能没有 Hook 自动注入后两层，所以 bedagent 帮你加了加载链提醒——首次使用时请确认 L2（think.md）和 L3（rules.md）是否都已读到。如果某层没读到，对话中会有提醒。

每次对话开始时，Agent 内部检查：
- L1 本文件（SKILL.md）— 当前 skill 调用已加载
- L2 think.md（{BEDAGENT_DATA}/think.md）— 需主动 Read
- L3 rules.md（~/.openclaw/skills/bedagent/rules.md 或等效路径）— 需主动 Read

如发现 L2 或 L3 未加载，在回复开头简短提醒用户：
「⚠️ 本次加载链第 X 层未加载，反思记忆/自定义规则不会生效。」

> 💡 **daemon 通知**：如果 `.bedagent/daemon-notice.md` 存在，Read 它——里面是 daemon 检测到的文件变化提醒（think.md / rules.md 已更新）。
