#!/bin/bash
# ============================================================
# bedagent compress-memory.sh · think.md 合并压缩脚本
# ============================================================
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help)
      echo "bedagent compress-memory v${VERSION}"
...
# ── 定位 think.md ──
# v0.90 P0-3 修复：BEDAGENT_DATA 由 lib/config.sh 统一解析（已 source）
THINK_FILE="${BEDAGENT_DATA}/think.md"
ARCHIVE_FILE="${BEDAGENT_DATA}/think.archive.md"
...
BACKUP_FILE="${BEDAGENT_DATA}/think.${BACKUP_DATE}.bak"
cp "$THINK_FILE" "$BACKUP_FILE"
ok "已备份: think.${BACKUP_DATE}.bak"

# ── 仅保留最近 3 份备份 ──
BACKUP_COUNT=0
for bak in "${BEDAGENT_DATA}"/think.*.bak; do
  [ -f "$bak" ] || continue
  BACKUP_COUNT=$((BACKUP_COUNT + 1))
done
if [ "$BACKUP_COUNT" -gt 3 ]; then
  # 按文件名排序（日期倒序），删除最旧的
  ls -1 "${BEDAGENT_DATA}"/think.*.bak 2>/dev/null | sort -r | tail -n +4 | while IFS= read -r old_bak; do
...
SIXTY_DAYS_AGO=$(date -v-60d +%Y-%m-%d 2>/dev/null || date -d '60 days ago' +%Y-%m-%d 2>/dev/null || echo "")
if [ -n "$SIXTY_DAYS_AGO" ]; then
  TMP_ACTIVE="${TMPDIR:-/tmp}/bedagent-think-active-$$.md"
  TMP_ARCHIVE_ADD="${TMPDIR:-/tmp}/bedagent-think-archive-add-$$.md"
