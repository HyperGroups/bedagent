#!/bin/bash
# ============================================================
# bedagent cleanup.sh · 数据保留清理脚本
# ============================================================
# 按保留策略清理 .bedagent/task/logs/ 下的过期日志。
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version) echo "bedagent-cleanup v${VERSION}"; exit 0 ;;
...
if [ "$SHOW_HELP" = true ]; then
  echo "bedagent cleanup v${VERSION}"
  echo "  按保留策略清理 .bedagent/task/logs/ 下的过期日志"
  echo ""
  echo "  用法:"
...
# ── 配置默认值 ──
RETENTION_DAYS="${BED_RETENTION_DAYS:-90}"
RETENTION_MAX="${BED_RETENTION_MAX:-500}"

# ── 路径 ──
LOGS_DIR="${PWD}/.bedagent/task/logs"
ARCHIVE_DIR="${PWD}/.bedagent/task/logs/archive"
