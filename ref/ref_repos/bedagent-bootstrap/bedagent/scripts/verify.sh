#!/bin/bash
# ============================================================
# bedagent verify.sh · 装后验证脚本
# ============================================================
# 验证 bedagent 安装完整性（9 个检查类别，24+ 项）
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help)
      echo "bedagent verify v${VERSION}"
      echo "  正常模式 彩色终端，显示所有检查项"
      echo "  --json   JSON 机器可读输出（CI/CD 用）"
      echo "  --quiet  只输出失败和警告，全通过时静默"
      echo "  --quick  快速模式——仅 4 项核心检查（SKILL.md / .bedagent/ / ao compose / rules.md）"
      echo "  --help   显示此帮助"
...
if [ "$QUICK_MODE" = true ]; then
  ...
  SKILL_QUICK="${OPENCLAW_DIR:-$HOME/.openclaw}/skills/bedagent/SKILL.md"
  ...
  if [ -d "${PWD}/.bedagent" ]; then
    check_pass ".bedagent/ 数据目录存在"
  else
    check_warn ".bedagent/ 数据目录不存在（首次使用会自动创建）"
  fi
  ...
  RULES_QUICK=""
  for c in \
    "${OPENCLAW_DIR:-$HOME/.openclaw}/skills/bedagent/rules.md" \
    "${HOME}/.workbuddy/skills/bedagent/rules.md" \
...
if [ "$PLATFORM" = "workbuddy" ]; then
  ...
  # WorkBuddy 专属检查（v0.62：宪法内联在 SKILL.md，检查 SKILL.md 而非 bedagent.md）
  if [ -f "$HOME/.workbuddy/skills/bedagent/SKILL.md" ] && [ -s "$HOME/.workbuddy/skills/bedagent/SKILL.md" ]; then
    if grep -q "4 底线\|10 铁律" "$HOME/.workbuddy/skills/bedagent/SKILL.md" 2>/dev/null; then
      check_pass "SKILL.md 已部署且含宪法（4底线+10铁律内联）"
    else
      check_warn "SKILL.md 已部署但宪法内容缺失"
    fi
  else
    check_warn "SKILL.md 未部署到 ~/.workbuddy/skills/bedagent/"
  fi
  ...
  if [ -d "$HOME/.workbuddy/skills/bedagent" ]; then
    count=$(ls -1 "$HOME/.workbuddy/skills/bedagent"/*.md 2>/dev/null | wc -l | tr -d ' ')
...
JSONEOF
    ...
    echo ""
    if [ "$fail" -eq 0 ]; then
      echo "  ✅ bedagent WorkBuddy 部署验证通过！"
      echo ""
      echo "  下一步:"
      echo "    1. 确认 bedagent Skill 已加载（下次对话应出现初始化提示）"
...
f="rules.md"
# v0.73: rules.md 部署到 skills/bedagent/rules.md（扁平化）
path="${OPENCLAW_DIR}/skills/bedagent/${f}"
if [ ! -f "$path" ]; then
...
if [ "$PLATFORM" != "openclaw" ]; then
  ...
  HOOK_DIR="${OPENCLAW_DIR}/hooks/bedagent-load-chain"
  HOOK_FILES_OK=0
  ...
  if [ "$HOOK_FILES_OK" = "2" ]; then
    check_pass "hook 目录就绪: hooks/bedagent-load-chain/（HOOK.md + handler.ts）"
  else
  ...
  if [ -f "$OC_CONFIG" ]; then
    if grep -q '"bedagent-load-chain"' "$OC_CONFIG" 2>/dev/null; then
      check_pass "openclaw.json 已注册 bedagent-load-chain hook"
    else
      check_warn "openclaw.json 未注册 bedagent-load-chain（加载链第 2、3 层不会自动注入）"
    fi
  else
  ...
  # v0.73: rules.md 权威路径 skills/bedagent/rules.md（扁平化）
  # ~/.openclaw/rules.md 是用户自定义文件，不再作为 bedagent 部署路径检查
  RULES_AUTHORITY="${OPENCLAW_DIR}/skills/bedagent/rules.md"
  if [ -f "$RULES_AUTHORITY" ]; then
    ...
    LEGACY_CONST="${OPENCLAW_DIR}/skills/bedagent/constitution/rules.md"
    if [ -f "$LEGACY_CONST" ]; then
      check_warn "  发现 v0.72 前安装残留（${LEGACY_CONST}）——建议运行 install.sh 升级，旧路径将自动迁移"
    fi
  fi
  # think.md 检查
  THINK_FILE="${PWD}/.bedagent/think.md"
  ...
  # 扫描 OpenClaw 启动日志，确认 bedagent-load-chain hook 被 agent:bootstrap 触发，
  ...
  if [ -d "$OPENCLAW_LOG_DIR" ]; then
    ...
    if [ -n "$RECENT_LOGS" ]; then
      ...
      for log_file in $RECENT_LOGS; do
        ...
        if echo "$LOG_CONTENT" | grep -q "bedagent-load-chain"; then
          HOOK_TRIGGERED=1
        fi
      ...
      if [ "$HOOK_TRIGGERED" = "1" ]; then
        check_pass "handler.ts 回归：bedagent-load-chain hook 已被触发"
        ...
        check_warn "handler.ts 回归：bedagent-load-chain hook 在最近日志中未检测到触发"
      fi
    else
...
# v0.90 P0-3 修复：BEDAGENT_DATA 已由顶部 config.sh 统一解析
if [ -d "$BEDAGENT_DATA" ]; then
  check_pass ".bedagent/ 数据目录存在"
  # 检查子目录
  for sub in task/logs orchestrator; do
    if [ -d "${BEDAGENT_DATA}/${sub}" ]; then
      check_pass "  .bedagent/${sub}/ 就绪"
    else
      check_warn "  .bedagent/${sub}/ 缺失"
    fi
  done
else
  check_warn ".bedagent/ 数据目录不存在（首次使用会自动创建）"
...
SKILL_FILE="${OPENCLAW_DIR:-$HOME/.openclaw}/skills/bedagent/SKILL.md"
...
if [ -d ".bedagent/task/logs" ]; then
  recent_count=$(find ".bedagent/task/logs" -name "*.md" -mtime -7 2>/dev/null | wc -l | tr -d ' ')
...
if [ -f ".bedagent/think.md" ]; then
  # GNU stat (-c %Y) 优先，BSD/macOS (-f %m) 回退；原 BSD-only 写法在 Linux 上恒返回 0 → 永远报"超旧"
  modified_sec=$(($(date +%s) - $(stat -c %Y ".bedagent/think.md" 2>/dev/null || stat -f %m ".bedagent/think.md" 2>/dev/null || echo 0)))
...
if [ "${BED_SANITIZE:-}" != "true" ] && [ "${BED_AUDIT_ENABLED:-}" != "true" ] && [ "${BED_CLEANUP_ON_RECORD:-}" != "true" ]; then
  check_pass "默认关闭: 合规功能全部关闭（向后兼容）"
else
  if [ "${BED_SANITIZE:-}" = "true" ]; then
    check_warn "脱敏已启用 (log_sanitize=true)"
  fi
  if [ "${BED_AUDIT_ENABLED:-}" = "true" ]; then
    check_warn "审计已启用 (audit_enabled=true)"
  fi
  if [ "${BED_CLEANUP_ON_RECORD:-}" = "true" ]; then
    check_warn "清理触发已启用 (data_cleanup_on_record=true)"
  fi
fi

# 10.5 rules.md 配置段完整性
# v0.73: 权威路径为 skills/bedagent/rules.md（扁平化）
# 兼容 fallback：工作目录（开发态）/ 旧部署路径（老安装）
RULES_FILE=""
for candidate in \
  "${PWD}/bedagent/rules.md" \
  "$HOME/.openclaw/skills/bedagent/rules.md" \
  "$HOME/.workbuddy/skills/bedagent/rules.md" \
  "${PWD}/bedagent/constitution/rules.md" \
  "$HOME/.openclaw/skills/bedagent/constitution/rules.md" \
  "$HOME/.workbuddy/skills/bedagent/constitution/rules.md"; do
...
# v0.90 P0-3 修复：BEDAGENT_DATA 已由顶部 config.sh 统一解析（不覆盖）
DAEMON_PID_FILE="${BEDAGENT_DATA}/daemon.pid"
DAEMON_JSON="${BEDAGENT_DATA}/daemon.json"

# daemon 是否安装
DAEMON_SCRIPT="${OPENCLAW_DIR}/scripts/daemon.sh"
[ ! -f "$DAEMON_SCRIPT" ] && DAEMON_SCRIPT="${PWD}/bedagent/scripts/daemon.sh"
...
if [ "$fail" -eq 0 ]; then
  [ "$JSON_MODE" = false ] && [ "$QUIET_MODE" = false ] && {
    echo "  ✅ bedagent 安装验证通过！"
    echo ""
    case "$PLATFORM" in
      openclaw)
        echo "  下一步:"
        echo "    1. 注册 before_prompt_build Hook（见 install.sh 输出）"
        echo "    2. 启动 OpenClaw，检查 system prompt 是否包含 bedagent 底线规则"
      ...
      claude|codex|hermes)
        ...
        echo "    2. 在下一轮对话中回复「bedagent」验证加载"
        ;;
    esac
  }
