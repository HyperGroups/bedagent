#!/usr/bin/env bash
# ============================================================
# bedagent verify-evidence.sh · 最小可信验证器 · v0.85
# ============================================================
# 扫描 .bedagent/task/logs/ 下今日记录，检查有无客观证据
# （测试 exit code / lint 结果），有标 [已验证]，无标 [未验证]。
#
# 用法：bash bedagent/scripts/verify-evidence.sh [--daemon]
...
TODAY=$(date +"%Y-%m-%d")
MONTH=$(date +"%Y-%m")
LOG_FILE="${PWD}/.bedagent/task/logs/${MONTH}/${TODAY}.md"

[ "$DAEMON_MODE" = false ] && echo "bedagent verify-evidence v${VERSION}"
