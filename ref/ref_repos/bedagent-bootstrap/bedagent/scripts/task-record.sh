#!/bin/bash
# ============================================================
# bedagent task-record.sh · 任务记录脚本
# ============================================================
...
#
# 输出位置：
#   .bedagent/task/logs/YYYY-MM/YYYY-MM-DD.md
#
# 用法：
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version) echo "bedagent-task-record v${VERSION}"; exit 0 ;;
    --help)
      echo "bedagent task-record v${VERSION}"
...
if [ "$IS_CLOSURE_CHECK" = true ]; then
  TODAY=$(date +"%Y-%m-%d")
  MONTH=$(date +"%Y-%m")
  LOG_DIR="${PWD}/.bedagent/task/logs/${MONTH}"
...
TODAY=$(date +"%Y-%m-%d")
MONTH=$(date +"%Y-%m")
LOG_DIR="${BEDAGENT_DATA}/task/logs/${MONTH}"
LOG_FILE="${LOG_DIR}/${TODAY}.md"
...
sanitize() {
  ...
  # 8. 内网 IP（可选，BED_SANITIZE_IPS=true 时启用）
  if [ "${BED_SANITIZE_IPS:-}" = "true" ]; then
