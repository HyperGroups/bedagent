...
> **闭环前先写 task/logs**（`{OPENCLAW_SCRIPTS}` 优先 `~/.openclaw/scripts/`，不存在则搜 `bedagent/scripts/`）：
> ```bash
...
>   --skills "task-aware"
> ```
> bash 不可用时降级为 LLM 直接追加写入 `{BEDAGENT_DATA}/task/logs/YYYY-MM/YYYY-MM-DD.md`（格式参考 `data/task.md`）。
