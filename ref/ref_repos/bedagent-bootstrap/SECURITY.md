...
bedagent 是纯本地治理层，**数据不出本机**——但以下数据以**明文 Markdown** 存储，请评估风险：

| 文件 | 位置 | 可能含 |
|------|------|------|
| `task/logs/` | `.bedagent/task/logs/YYYY-MM/YYYY-MM-DD.md` | 任务摘要、代码片段、API 响应摘要、对话摘要 |
| `think.md` | `.bedagent/think.md` | 反思记录，可能含踩坑细节、失败模式、决策推理 |
| `scoring/` | `.bedagent/scoring/` | Skill 使用记录 |
| `orchestrator/` | `.bedagent/orchestrator/` | 编排决策历史 |
...
- ⚠️ 明文存储：`.bedagent/` 下文件仍为 Markdown 明文，未做加密
- ⚠️ **企业生产环境暂不建议使用**——数据明文存储 + LLM 自评无外部基准，GDPR / 等保 / SOC2 场景直接出局。age 加密推到 v1.x+ 待评估（v0.85 砍削决策：先验证核心价值再谈企业级）
- `.bedagent/` 目录权限为 700（仅当前用户可访问），但同一服务器其他用户若有 root 权限可读

**企业环境建议**：
- 对 `.bedagent/` 目录做 gpg 加密或放在加密卷上
...
- **GitHub 私密 Issue**：[新建 Issue](https://github.com/HyperGroups/bedagent/issues) 时选 "Security" 标签
...
本安全策略适用于 bedagent 项目仓库内的所有文件。第三方依赖（如 agency-orchestrator、OpenClaw）的安全问题请向对应项目报告。

---

## 第三方依赖供应链

**agency-orchestrator (ao)** 是 bedagent 编排引擎的运行时依赖（npm 包 `agency-orchestrator@0.7.5`，Apache-2.0 许可）。install.sh 已将版本号 pin 到具体版本。
