#!/usr/bin/env node
// ============================================================
// bedagent-audit · 提交时审计 CLI 入口
// v0.91 · 由 DeepSeek V4 Pro 和 GLM-5.2 配合生成
// ============================================================
// 扫描 git diff，检查 Agent 是否遵守 bedagent 铁律。
// 零运行时依赖——只用 Node.js 内置模块。
//
// 用法：
//   node bedagent-audit --diff HEAD~1..HEAD --task "修复登录页 bug"
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--diff' && argv[i + 1]) {
      ...
      console.log('bedagent-audit v0.91 · 提交时审计\n');
      console.log('用法: bedagent-audit --diff <range> [--task <description>]');
      console.log('  --diff    git diff 范围（默认 HEAD~1..HEAD）');
      console.log('  --task    任务描述（用于铁律 #7 谨慎修改检查）');
      console.log('退出码: 0=全通过 / 1=有警告 / 2=有违规');
      process.exit(0);
    } else if (argv[i] === '--version') {
      console.log('bedagent-audit v0.91');
      process.exit(0);
    }
  }
  return args;
...
function printResults(results: AuditResult, diffFiles: DiffFile[]): void {
  console.log(`\n[bedagent-audit] 扫描 ${diffFiles.length} 个变更文件\n`);
...
main().catch((err) => {
  console.error('bedagent-audit 内部错误:', err.message);
  process.exit(2);
});
