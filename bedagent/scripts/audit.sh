#!/bin/bash
# ============================================================
# bedagent audit.sh · 审计日志脚本
# ============================================================
# 记录关键操作（install / uninstall / orchestrate / cleanup / record）
# 到 .bedagent/task/audit/YYYY-MM/YYYY-MM-DD.md，追加 Markdown 表格行。
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version)   echo "bedagent-audit v${VERSION}"; exit 0 ;;
    --help)
      echo "bedagent audit v${VERSION}"
      echo "  记录关键操作到 .bedagent/task/audit/YYYY-MM/YYYY-MM-DD.md"
      echo ""
      echo "  用法:"
...
# ── 审计开关检查 ──
# 仅 BED_AUDIT_ENABLED=true 时写入，未配置时静默退出
...
# ── 路径 ──
# v0.90 P0-3 修复：BEDAGENT_DATA 由 lib/config.sh 统一解析（已 source），不再硬编码 ${PWD}
AUDIT_DIR="${BEDAGENT_DATA}/task/audit/${LOCAL_MONTH}"
