#!/bin/bash
# ============================================================
# bedagent daemon.sh · daemon 主进程 · v0.85
# ============================================================
...
BEDAGENT_DATA="${REPO_ROOT}/.bedagent"
DAEMON_JSON="${BEDAGENT_DATA}/daemon.json"
DAEMON_LOG="${BEDAGENT_DATA}/daemon.log"
DAEMON_PID_FILE="${BEDAGENT_DATA}/daemon.pid"

_ensure_data_dir() {
  mkdir -p "$BEDAGENT_DATA"
}
...
_find_think() {
  local f="${REPO_ROOT}/.bedagent/think.md"
  [ -f "$f" ] && { echo "$f"; return 0; }
  echo ""
}
  ...
  while true; do
    ...
    if [ -n "$think_hash" ] && [ "$think_hash" != "${old_think:-}" ]; then
      ...
      echo "[daemon] $(date -u +"%Y-%m-%dT%H:%M:%SZ") think.md 已变更——下次启动时建议读取最新反思" \
        > "${BEDAGENT_DATA}/daemon-notice.md"
    fi
...
case "${1:-}" in
  ...
  --foreground)
    ...
    ;;
  *)
    echo "bedagent daemon v${VERSION}"
    echo ""
