#!/bin/bash
# ============================================================
# bedagent install.sh · 多平台一键安装脚本
# ============================================================
# 将 bedagent 约束层部署到目标平台，让 Agent 获得治理能力。
# 由 DeepSeek V4 Pro 和 GLM-5.2 配合生成。
#
# 平台支持：
#   --platform openclaw  → 完整部署（宪法 + Hook + 脚本 + 断路器）
#   --platform workbuddy → 检查 .bedagent/ 数据目录 + 运行 verify.sh（SKILL.md 入口流程自动管理）
...
info()  { echo -e "${BLUE}[bedagent]${NC} $1"; }
...
echo ""
echo "  ╔═══════════════════════════════════╗"
echo "  ║   bedagent Harness · installer   ║"
echo "  ╚═══════════════════════════════════╝"
echo ""
fi
...
if [ "${REMOTE_MODE}" = "1" ]; then
  info "远程安装模式——克隆仓库..."
  REMOTE_TMP="$(mktemp -d /tmp/bedagent-remote-XXXXXX)"
  if command -v git &>/dev/null; then
    git clone https://github.com/HyperGroups/bedagent.git "$REMOTE_TMP" 2>/dev/null || {
    ...
    exec bash bedagent/scripts/install.sh $REMAINING_ARGS
  else
    err "git 不可用——远程安装需要 git。请先安装 git 或使用完整安装方式："
    err "  git clone https://github.com/HyperGroups/bedagent.git && cd bedagent && bash bedagent/scripts/install.sh"
    exit 1
  fi
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  workbuddy 检查 .bedagent/ 数据目录 + 运行 verify.sh"
      ...
      echo "  --project-dir DIR   指定项目工作目录（.bedagent/ 数据目录会创建在这里，默认当前目录）"
...
if [ -n "${PROJECT_DIR:-}" ]; then
  ...
  ok "数据目录: ${PROJECT_DIR}/.bedagent/"
else
  PROJECT_DIR="$PWD"
  warn "未指定 --project-dir，.bedagent/ 数据目录将创建在当前目录: ${PROJECT_DIR}"
  warn "  如果这不是你的项目工作目录，请用 --project-dir 指定："
  warn "  bash bedagent/scripts/install.sh --project-dir ~/my-project"
fi

# ── 统一初始化数据目录路径（所有平台共用，避免 set -u 下未定义）──
BEDAGENT_DATA="${PROJECT_DIR}/.bedagent"

# v0.90 P0-3 修复：写入数据目录标记文件，供 audit/verify/orchestrate 等脚本定位
# 标记文件放在平台 skill 目录下，config.sh 读取它来还原 --project-dir 指定的路径
case "$PLATFORM" in
  openclaw|workbuddy)
    _SKILL_DIR="${TARGET:-${HOME}/.${PLATFORM}/skills/bedagent}"
    mkdir -p "$_SKILL_DIR" 2>/dev/null || true
    echo "$BEDAGENT_DATA" > "$_SKILL_DIR/.bedagent-data-path" 2>/dev/null || true
    ;;
esac
...
case "$PLATFORM" in
  ...
  workbuddy)
    ...
    if [ -d "$BEDAGENT_DATA" ]; then
      ...
      warn "  · .bedagent/ 不存在——下次加载 bedagent Skill 时自动创建"
    fi
...
# 初始化安装日志
mkdir -p "$TARGET"
INSTALL_LOG="${TARGET}/.bedagent-install.log"
echo "" >> "$INSTALL_LOG"
echo "=== bedagent install $(date -u +'%Y-%m-%dT%H:%M:%SZ') ===" >> "$INSTALL_LOG"
_log "TARGET=$TARGET"
...
# 检查源文件
if [ ! -f "$RULES_SRC" ]; then
  err "找不到 rules.md。请在 bedagent 项目根目录下运行此脚本。"
...
# OpenClaw: rules.md 统一部署到 skills/bedagent/（~/.openclaw/rules.md 留给用户自定义）
# 其他平台: rules.md 部署到 $TARGET/rules.md
if [ "$PLATFORM" = "openclaw" ]; then
  RULES_DST_DIR="${TARGET}/skills/bedagent"
  ...
  OLD_RULES="${TARGET}/skills/bedagent/constitution/rules.md"
  if [ -f "$OLD_RULES" ]; then
...
# ════════════════════════════════════════
info "Step 5/7 · 部署 Skill 文件 → $TARGET/skills/bedagent"

SKILL_DST="${TARGET}/skills/bedagent"
mkdir -p "$SKILL_DST"

copied=0

# 核心 Skill 文件（从 bedagent/ 根目录复制）
...
if [ "${LITE_MODE:-0}" = "1" ]; then
  mkdir -p "$BEDAGENT_DATA"  # v0.90 P0-2 修复：Lite 跳过 Step 5b，需提前创建数据目录
  THINK_DST="${BEDAGENT_DATA}/think.md"
  if [ ! -f "$THINK_DST" ]; then
    cat > "$THINK_DST" << 'THINK_TEMPLATE'
# 反思区（think.md）

> bedagent 反思区——自动记录每次任务的教训和经验。
...
# ════════════════════════════════════════
# P0-2/P0-3 修复：配套脚本部署和 .bedagent/ 数据目录创建不再限于 OpenClaw，
...
# 创建 .bedagent/ 数据目录（BEDAGENT_DATA 已在平台分支前统一初始化）
if [ ! -d "$BEDAGENT_DATA" ]; then
  mkdir -p "$BEDAGENT_DATA/task/logs" "$BEDAGENT_DATA/orchestrator/workflows"
  chmod 700 "$BEDAGENT_DATA" 2>/dev/null || true  # 权限加固：仅当前用户可访问
  ok "数据目录已创建: $BEDAGENT_DATA"
else
  ok "数据目录已存在: $BEDAGENT_DATA"
fi
...
# ~/.openclaw/hooks/bedagent-load-chain/，并在 openclaw.json 的
# hooks.internal.entries.bedagent-load-chain 注册 enabled:true，即自动生效。
# 旧版 load-chain.sh（config.json.before_prompt_build shell hook）已废弃，不再部署。

HOOK_SRC_DIR="${SCRIPT_DIR}/../hooks/bedagent-load-chain"
HOOK_DST_DIR="${TARGET}/hooks/bedagent-load-chain"
...
if [ -d "$HOOK_SRC_DIR" ] && [ -f "${HOOK_SRC_DIR}/HOOK.md" ] && [ -f "${HOOK_SRC_DIR}/handler.ts" ]; then
  ...
  # ── 在 openclaw.json 注册 hooks.internal.entries.bedagent-load-chain ──
  ...
  if [ -f "$HOOK_CONFIG" ] && grep -q '"bedagent-load-chain"' "$HOOK_CONFIG" 2>/dev/null; then
  ...
  if [ "$ALREADY_REGISTERED" = "1" ]; then
    ...
    if command -v jq &>/dev/null; then
      # jq 合并 hooks.internal.entries.bedagent-load-chain = {enabled:true}
      # P0-1 修复：jq 输出后检查 .tmp 非空再 mv，避免空文件覆盖配置
      jq '.hooks.internal.enabled = ((.hooks.internal.enabled // false) or true) | .hooks.internal.entries = ((.hooks.internal.entries // {}) + {"bedagent-load-chain": {"enabled": true}})' \
...
cfg.hooks.internal.entries['bedagent-load-chain'] = { enabled: true };
...
HOOK_INJECT
    ...
    if [ "$REGISTER_OK" = "1" ]; then
      ok "Hook 已自动注册（hooks.internal.entries.bedagent-load-chain）"
    else
      warn "请手动在 $HOOK_CONFIG 添加："
      warn '  {"hooks":{"internal":{"enabled":true,"entries":{"bedagent-load-chain":{"enabled":true}}}}}'
    fi
  fi
else
  warn "找不到 hook 源文件（$HOOK_SRC_DIR/HOOK.md 或 handler.ts），跳过部署"
  warn "  仓库结构异常？请从 https://github.com/HyperGroups/bedagent 重新拉取"
fi
...
if [ "$PLATFORM" = "claude" ] || [ "$PLATFORM" = "codex" ] || [ "$PLATFORM" = "hermes" ]; then
  ...
  SEED_CONTENT="每次对话开始时，读取以下文件并执行 bedagent 入口流程：
1. rules.md：${SEED_PLATFORM_LABEL}（宪法已在 SKILL.md 内联）
2. 如果工作目录含 .bedagent/ 数据文件，加载记忆和反思
如果数据文件（.bedagent/）不存在，先创建空模板。"

  # P1-5: 自动写入种子指令（查重：已含 bedagent 则跳过）
  if [ -f "$SEED_FILE" ] && grep -q 'bedagent' "$SEED_FILE" 2>/dev/null; then
  ...
  echo "  ── 写入内容 ──"
  echo ""
  echo "  每次对话开始时，读取以下文件并执行 bedagent 入口流程："
  echo "  1. rules.md：${SEED_PLATFORM_LABEL}（宪法已在 SKILL.md 内联）"
  echo "  2. 如果工作目录含 .bedagent/ 数据文件，加载记忆和反思"
  echo "  如果数据文件（.bedagent/）不存在，先创建空模板。"
  echo ""
  echo "  💡 在下一轮对话中回复「bedagent」验证是否加载成功。"
  echo ""
fi
...
case "$PLATFORM" in
  openclaw)
    echo "  已部署文件："
    echo "    宪法文件:      $TARGET/skills/bedagent/rules.md（宪法内联在 SKILL.md）"
    echo "    Skill 文件:     $TARGET/skills/bedagent/（6 核心 + 4 数据模板）"
    echo "    加载链 Hook:    $TARGET/hooks/bedagent-load-chain/（HOOK.md + handler.ts）"
...
if [ -f "${HOOK_CONFIG:-}" ] && grep -q '"bedagent-load-chain"' "$HOOK_CONFIG" 2>/dev/null; then
  ...
  echo '     {"bedagent-load-chain":{"enabled":true}}'
fi
...
if [ "${REMOTE_MODE:-0}" = "1" ]; then
  DAEMON_INSTALL_SCRIPT="${REMOTE_TMP}/bedagent/scripts/daemon-install.sh"
fi
  ...
  case "$OS_TYPE" in
    Darwin|Linux)
      ...
      if [ "$QUICK_MODE" = "1" ] || [ "$NO_DAEMON" = "1" ]; then
        echo ""
        echo "  ⏭️  跳过 daemon 安装"
        echo "  （以后可以手动运行: bash bedagent/scripts/daemon-install.sh）"
      else
        echo ""
        echo "  ┌──────────────────────────────────────────┐"
        ...
        if [ "${INSTALL_DAEMON:-n}" = "y" ] || [ "${INSTALL_DAEMON:-n}" = "Y" ]; then
          ...
          echo "  已跳过 daemon 安装（以后可以手动运行: bash bedagent/scripts/daemon-install.sh）"
        fi
      fi
      ;;
    *)
