#!/bin/bash
# ============================================================
# bedagent daemon-install.sh · daemon 安装脚本 · v0.85
# ============================================================
...
case "$OS" in
  Darwin)
    ...
    mkdir -p "$PLIST_DIR"
    PLIST_FILE="$PLIST_DIR/com.bedagent.daemon.plist"
...
<dict>
    ...
    <string>${REPO_ROOT}/.bedagent/daemon.log</string>
...
PLISTEOF
  ...
  Linux)
    ...
    mkdir -p "$SYSTEMD_DIR"
    SERVICE_FILE="$SYSTEMD_DIR/bedagent-daemon.service"

    cat > "$SERVICE_FILE" << SERVICEEOF
[Unit]
Description=bedagent daemon
...
SERVICEEOF
    ...
    systemctl --user enable bedagent-daemon.service 2>/dev/null || true
    systemctl --user start bedagent-daemon.service 2>/dev/null
