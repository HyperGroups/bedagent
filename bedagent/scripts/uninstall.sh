#!/bin/bash
# ============================================================
# bedagent uninstall.sh · 卸载脚本
# ============================================================
# 删除 bedagent 约束文件，但保留 .bedagent/ 用户数据。
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help)
      echo "bedagent uninstall [--platform openclaw|workbuddy|claude|codex|hermes]"
      echo "  正常模式 交互确认后删除约束文件"
      echo "  --force  跳过确认，直接删除"
      echo "  --list   仅列出会被删除的文件，不执行"
      echo "  --platform 指定目标平台（未指定时自动探测）"
      echo "  保留: .bedagent/ 数据目录（task-record / orchestrator）"
      exit 0 ;;
  esac
done
...
case "$PLATFORM" in
  ...
  workbuddy)
    # v0.90 P0-3 修复：BEDAGENT_DATA 由 config.sh 统一解析
    echo "WorkBuddy 平台——准备清理 bedagent 部署文件"
    echo ""
    removed=0
    ...
    # 兼容旧 skills/bedagent/ 路径
    skill_path="$HOME/.workbuddy/skills/bedagent/$f"
    if [ -f "$skill_path" ]; then
      if [ "$LIST_ONLY" = true ]; then info "  $skill_path"; else rm -f "$skill_path" && ok "已删除: skills/bedagent/$f"; fi
    fi
    # 兼容旧 constitution/ 路径
    old_path="$HOME/.workbuddy/skills/bedagent/constitution/$f"
    if [ -f "$old_path" ]; then
    ...
    # 清理旧版遗留的 bedagent.md（v0.62 前部署的宪法文件）
    legacy="$HOME/.workbuddy/bedagent.md"
    if [ -f "$legacy" ]; then
      if [ "$LIST_ONLY" = true ]; then
        info "  $legacy（旧版遗留）"
      else
    ...
    # 清理 Skill 目录
    skill_dir="$HOME/.workbuddy/skills/bedagent"
    if [ -d "$skill_dir" ]; then
      ...
      if [ "$LIST_ONLY" = true ]; then
        ...
        rm -rf "$skill_dir"
        ok "已删除 skills/bedagent/ 目录（${skill_count} 个文件）"
      fi
      ((removed++)) || true
    fi
    ...
    if [ "$LIST_ONLY" = true ]; then
      ...
      echo "  工作区数据 .bedagent/ 保留（需手动 rm -rf 清除）。"
      exit 0
    fi
    ...
    echo "───────────────────────────────────────"
    echo ""
    echo "  bedagent WorkBuddy 部署文件已清理。"
    if [ -d "$BEDAGENT_DATA" ]; then
      echo "  工作区数据保留在: ${BEDAGENT_DATA}（如需清除请手动 rm -rf）"
    fi
    echo ""
    exit 0
    ;;
...
# v0.90 P0-3 修复：BEDAGENT_DATA 已由 config.sh 统一解析（不覆盖）

echo ""
echo "  ╔═══════════════════════════════════╗"
echo "  ║   bedagent · uninstall           ║"
echo "  ╚═══════════════════════════════════╝"
echo ""
echo "  平台: $PLATFORM"
echo "  将从以下位置删除 bedagent 文件："
echo "    $TARGET"
echo ""
echo "  保留用户数据："
echo "    $BEDAGENT_DATA"
...
# ── 删除 / 列出宪法文件（v0.73：rules.md 扁平化到 skills/bedagent/rules.md）──
f="rules.md"
# 新路径
...
if [ -f "$path" ]; then
  if [ "$LIST_ONLY" = true ]; then
    ...
    rm -f "$path" "${path}.bak"
    ok "已删除: skills/bedagent/$f"
  fi
  ((removed++)) || true
fi
# 兼容旧 constitution/ 路径
old_path="${OPENCLAW_DIR}/skills/bedagent/constitution/${f}"
if [ -f "$old_path" ]; then
  if [ "$LIST_ONLY" = true ]; then
...
# ── 清理旧版遗留的 bedagent.md（v0.62 前部署的宪法文件）──
legacy="${OPENCLAW_DIR}/bedagent.md"
if [ -f "$legacy" ]; then
  if [ "$LIST_ONLY" = true ]; then
    info "  $legacy（旧版遗留）"
  else
    rm -f "$legacy" "${legacy}.bak"
    ok "已删除旧版遗留: bedagent.md"
  fi
  ((removed++)) || true
fi

# ── 删除 / 列出 Skill 文件 ──
SKILLS_DIR="${OPENCLAW_DIR}/skills/bedagent"
...
if [ -d "$HOOK_DIR" ]; then
  if [ "$LIST_ONLY" = true ]; then
    ...
    ok "已删除: hooks/bedagent-load-chain/"
  fi
  ((removed++)) || true
fi
...
if [ -f "$OC_CONFIG" ] && command -v jq &>/dev/null; then
  if jq -e '.hooks.internal.entries."bedagent-load-chain"' "$OC_CONFIG" >/dev/null 2>&1; then
    if [ "$LIST_ONLY" = true ]; then
      info "  $OC_CONFIG (hooks.internal.entries.bedagent-load-chain)"
    else
      jq 'del(.hooks.internal.entries."bedagent-load-chain")' "$OC_CONFIG" > "${OC_CONFIG}.tmp" 2>/dev/null
      mv "${OC_CONFIG}.tmp" "$OC_CONFIG" 2>/dev/null && ok "已注销 openclaw.json 中的 bedagent-load-chain hook"
    fi
    ((removed++)) || true
  fi
fi
...
if [ "$LIST_ONLY" = true ]; then
  echo ""
  echo "  共 ${removed} 项会被删除。数据目录 .bedagent/ 保留。"
  exit 0
fi
...
# ── 清理安装日志 ──
INSTALL_LOG="${OPENCLAW_DIR}/.bedagent-install.log"
rm -f "$INSTALL_LOG"
...
echo "───────────────────────────────────────"
echo ""
echo "  bedagent 约束文件已删除。"

if [ -d "$BEDAGENT_DATA" ]; then
  echo "  用户数据保留在: $BEDAGENT_DATA"
else
  echo "  （无用户数据需要保留）"
fi

echo ""
echo "  如需重新安装，运行: bash bedagent/scripts/install.sh --platform $PLATFORM"
echo ""
