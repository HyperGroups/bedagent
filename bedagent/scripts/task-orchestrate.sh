#!/bin/bash
# ============================================================
# bedagent task-orchestrate.sh · AO 编排包装脚本
# ============================================================
...
if ! command -v ao &>/dev/null; then
  echo "[bedagent] ⚠️ agency-orchestrator (ao) 未安装——编排引擎不可用"
  echo "[bedagent] 降级方案：手动拆任务 → 用 task-record.sh 逐条记录 → 手动闭环"
  echo "[bedagent] 安装 ao: npm install -g agency-orchestrator@0.7.5  或  加 --no-ao 参数跳过"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version)    echo "bedagent-task-orchestrate v${VERSION}"; exit 0 ;;
    --help)
      echo "bedagent task-orchestrate v${VERSION}"
...
echo ""
echo "  ╔═══════════════════════════════════╗"
echo "  ║   bedagent · task orchestrate    ║"
echo "  ╚═══════════════════════════════════╝"
echo ""
...
# v0.90 P0-3 修复：BEDAGENT_DATA 由 lib/config.sh 统一解析
ORCHESTRATOR_DIR="${BEDAGENT_DATA}/orchestrator"
...
analyze_track_record() {
  ...
  for logfile in "${BEDAGENT_DATA}"/task/logs/*/*/*.md; do
    [ -f "$logfile" ] || continue
...
sliding_window_rollback() {
  ...
  for logfile in "${BEDAGENT_DATA}"/task/logs/*/*/*.md; do
    ...
    awk -v task="$TASK_DESC" '
      ...
      in_block && /\| 状态/ { print }
    ' "$logfile"
  done | tail -5 > "${TMPDIR:-/tmp}/bedagent-rollback-$$.txt"
  while IFS= read -r line; do
    if echo "$line" | grep -q "成功"; then ((success_count++)) || true; fi
    ((total++)) || true
  done < "${TMPDIR:-/tmp}/bedagent-rollback-$$.txt"
  rm -f "${TMPDIR:-/tmp}/bedagent-rollback-$$.txt"
...
WORKFLOW_FILE="${TMPDIR:-/tmp}/bedagent-workflow-$$.yaml"
...
# 自动触发 bedagent-load-chain（注入 think.md + rules.md），第 1 层宪法由 skill 系统
...
# 清理
rm -f "$WORKFLOW_FILE" "${BEDAGENT_CONSTRAINT_FILE:-}"
